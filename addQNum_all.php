<?php
/**
 * addQNum_all.php - Queue/Extension Sync to Asterisk
 * ===================================================
 * Date: 2026-02-05
 * Version: 1.1.0
 *
 * v1.1.0 변경사항:
 * - PBX 필터링 추가: 내 장비에 해당하는 Q/Extension만 동기화
 * - pbx_config.php 사용하여 현재 PBX ID 자동 감지
 * - T_COMPANY JOIN으로 pbx_id 기반 필터링
 */

include 'plog.php';
include 'pbx_config.php';
...
***/
		...
		// v1.1.0: 내 PBX ID 조회
		$my_pbx_config = get_my_pbx_config($conn_db);
		$MY_PBX_ID = $my_pbx_config->pbx_id;

		WLOG( sprintf( '[IPPBXSET %s] MY_PBX_ID=%s, IP=%s', '-----------', $MY_PBX_ID, $my_pbx_config->server_ip ) );

		if ($MY_PBX_ID == '0') {
			WLOG( sprintf( '[IPPBXSET %s] ERROR: Cannot identify PBX, skipping sync', '-----------' ) );
			return "ERROR:Unknown PBX";
		}
		...
		// v1.1.0: 내 PBX에 해당하는 Q/Extension만 조회
		$sql = "SELECT qe.q_num, qe.ext_number, qe.call_order, qe.q_ext_id, qe.master_id
			FROM T_Q_EXTENSION qe
			INNER JOIN T_COMPANY c ON qe.master_id = c.master_id AND c.company_level = 0
			WHERE c.pbx_id = '$MY_PBX_ID'
			ORDER BY qe.q_num, qe.call_order;";
		...
		// v1.1.0: 내 PBX에 해당하는 Queue만 조회
		$sql = "SELECT q.q_num, q.hunt_type, q.hunt_time, q.is_other_q, q.other_q_num, q.master_id
			FROM T_QUEUE q
			INNER JOIN T_COMPANY c ON q.master_id = c.master_id AND c.company_level = 0
			WHERE c.pbx_id = '$MY_PBX_ID'
			ORDER BY q.q_num;";
		...
		for( $i=0; $i<$Q_CNT; $i++ )
		{
			...
			$conn = mysqli_connect(
                ...
                //error_log($NEWLIST);
                //WLOG( sprintf( '[ADD_QUEU ----:---] %s %s', $MIQLIST, $NEWLIST ) );
                ...
                error_log($sql);
                WLOG( sprintf( '[ADD_QUEU %s] %s', $MIQLIST, $sql ) );
                $res = mysqli_multi_query($conn, $sql);
		...
		$UPDATE_T_QE='';
                ...
                {       
                        ...
                        $DATA = 'PJSIP/'.$EXTENSION[$i].','.$ORD_VAL;
		...
		$sql = "update T_Q_EXTENSION set is_send_pbx='Y', request_pbx_datetime=now() where q_ext_id in ( $NEWLIST );update T_QUEUE set is_send_pbx='Y' where q_num in ( $NEWLIST2 );";
		...
		// v1.1.0: 내 PBX에 해당하는 Company만 조회
		$sql = "SELECT c2.use_queue, c2.ringo, c2.company_id, c2.master_id
			FROM T_COMPANY c2
			INNER JOIN T_COMPANY c0 ON c2.master_id = c0.master_id AND c0.company_level = 0
			WHERE c2.company_level = 2 AND c2.use_queue != '' AND c0.pbx_id = '$MY_PBX_ID'
			ORDER BY c2.ringo, c2.use_queue;";
