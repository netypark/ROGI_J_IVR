...
include 'plog.php';
	...
	function make_tts( $DIR, $TEXT, $FILE ) 
	{
		WLOG( sprintf( '[MAKE_TTS ] START =======================================================================================================' ) ); 

		$RESULT = "0";

		$RESULT = shell_exec( "/usr/bin/curl -X POST -H \"Content-Type: application/json; charset=utf-8\" -d '{\"TTS_SPEED\":100,\"FORMAT\":\"HTML\",\"REQUEST\":\"TTS_VW\",\"TTS_TEXT\":\"$TEXT\"}' http:\/\/127.0.0.1:8080\/API\/ > $DIR$FILE" );
		
		WLOG( sprintf( '[MAKE_TTS ] DEST %s%s', $DIR, $FILE ) ); 
		WLOG( sprintf( '[MAKE_TTS ] TEXT %s', $TEXT ) ); 

		WLOG( sprintf( '[MAKE_TTS ] END   =======================================================================================================' ) ); 
		return $RESULT;
	}
...
error_log(sprintf('+++++++++++++make_directory.php args->JSON_REQUEST [%s]', $args->JSON_REQUEST));
		...
		if (is_object($JSON_REQUEST)) {
			...
			if (isset($JSON_REQUEST->REQ)) {
				if ($JSON_REQUEST->REQ == 'MAKE_TTS') 
				{
				   	$JSON_API_RESULT->JSON_RESULT->CODE       = "TRY_CALL_PROCEDURE";

					$JSON_API_RESULT->JSON_RESULT->MESSAGE    = make_tts( $JSON_REQUEST->DIR,
												$JSON_REQUEST->TEXT, 
												$JSON_REQUEST->FILE );
				}
