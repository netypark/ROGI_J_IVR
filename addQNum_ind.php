...
***/
		...
		for( $i=0; $i<$Q_CNT; $i++ )
		{
			...
			if( $row = mysqli_fetch_array($res) )
			{
				...
				$sql = "update queues_config set dest='$DEST_Q' where extension='$QUEUE[$i]';update queues_details set data='$STRATEGY' where keyword='strategy' and id='$QUEUE[$i]';update queues_config set maxwait='$HUNT_TIME[$i]' where extension='$QUEUE[$i]';update queues_details set data='0' where keyword='timeout' and id='$QUEUE[$i]';";
