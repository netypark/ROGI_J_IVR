...
**/
		...
		else
		{
			...
			$sql = "select company_id, did_company_id, did_number, use_dnd, use_transfer, use_cid_route, use_db_route, is_use, COALESCE(NULLIF(to_number, ''), '0') AS result from T_DID_RANGE where did_number='$DID' limit 1;";
	...
	***/
					...
					$sql = "select c_level, start_datetime, end_datetime from T_CUSTOMER where ( company_id=$RESULT->company_id or master_id=$RESULT->master_id) and c_phone='$CID' and NOW() between date_format(start_datetime, '%Y-%m-%d %H:%i:%s') AND date_format(end_datetime, '%Y-%m-%d %H:%i:%s') limit 1;";
					$sql = "select c_level, start_datetime, end_datetime from T_CUSTOMER where ( ( c_level = 'V' AND company_id = '$RESULT->company_id' AND c_phone='$CID' ) or ( c_level = 'B' AND master_id = '$RESULT->master_id' AND c_phone='$CID' ) ) and NOW() between date_format(start_datetime, '%Y-%m-%d %H:%i:%s') AND date_format(end_datetime, '%Y-%m-%d %H:%i:%s') limit 1;";
					...
					if( $RESULT->c_level == 'V' )
					{
						$sql = "INSERT INTO T_VIP_COUNT ( company_id, master_id, vip_cnt, mod_datetime ) VALUES ( $RESULT->company_id, $RESULT->master_id, 1, now() ) ON DUPLICATE KEY UPDATE vip_cnt = vip_cnt + 1, mod_datetime = NOW();";

						SLOG( sprintf( '[SAV_VIPCNT %s:%s] %s', $DID, $CID, $sql ) );

						$res = mysqli_query($conn, $sql);

						$sql = "update T_COMPANY set monitering_vip_info='$CID', monitering_vip_datetime=now() where company_id=$RESULT->company_id and master_id=$RESULT->master_id and company_level=2;";

						SLOG( sprintf( '[UP_COMPANY %s:%s] %s', $DID, $CID, $sql ) );
					...
					if( $RESULT->use_db_route == 'Y' || $RESULT->use_cid_route == 'Y'  )
					{
						...
						else
						{
							if( $RESULT->use_cid_route == 'Y' )
							{
								$sql = "select to_extension from T_DB_ROUTING where company_id=$RESULT->company_id and from_did='$DID' and si_do='$SIDO' and kind='C' order by create_datetime limit 1;";
								error_log($sql);
								SLOG( sprintf( '[GET_CID_ROT %s:%s] %s', $DID, $CID, $sql ) );
					...
					if( $DR_TO_EXTEN_S == '0' )
					{
						...
						if( $RESULT->use_ars == 'Y' )
						{
							...
							$sql = "select h_memt_dir from T_HOLIDAY where h_company_id=$RESULT->company_id and DATE_FORMAT(curdate(), '%m%d') >= h_startdate and DATE_FORMAT(curdate(), '%m%d') <= h_enddate limit 1;";
						...
						$sql = "update T_COMPANY set call_update_time=now() where company_id=$RESULT->company_id;";
