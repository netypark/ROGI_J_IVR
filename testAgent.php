...
include 'plog.php';
	...
	function test_agent( ) 
	{ 
		SLOG( sprintf( '[TEST_AGENT] START =======================================================================================================' ) );

		$RESULT = new stdClass();

		$rand_num = sprintf('%02d',rand(00,99));
		$even_odd = $rand_num % 2;
		$this->F_PUT_VARIABLE('RAND_NUM', '$rand_num');
		$this->F_PUT_VARIABLE('EVEN_ODD', '$even_odd');
		$even_odd == 0 ? $CID='9999'.$CID:$CID='8888'.$CID;
		SLOG( sprintf( '%s', $even_odd) );
		error_log(sprintf('%s', $even_odd));
		SLOG( sprintf( '%s', $CID) );
		error_log(sprintf('%s', $CID));

		SLOG( sprintf( '[GET_AGENT] END   =======================================================================================================' ) );
		return $RESULT;
	}
...
//error_log(sprintf('+++++++++++++updateMonTbl.php args->JSON_REQUEST [%s]', $args->JSON_REQUEST));
		...
		if (is_object($JSON_REQUEST)) {
			...
			if (isset($JSON_REQUEST->REQ)) {
				if ($JSON_REQUEST->REQ == 'TEST_AGENT') 
				{
				   	$JSON_API_RESULT->JSON_RESULT->CODE       = "TRY_CALL_PROCEDURE";

				   	$JSON_API_RESULT->JSON_RESULT->MESSAGE          = test_agent();
				}
