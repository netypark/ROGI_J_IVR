...
include 'plog.php';
	...
	function setRingo( $MASTER_ID, $COMPANY_ID ) 
	{ 
		WLOG( sprintf( '[SET_RING %s:%s] %s', $MASTER_ID, $COMPANY_ID, 'START =======================================================================================================' ) );
...
***/
		...
		error_log($sql);
		WLOG( sprintf( '[SET_RING %s:%s] %s', $MASTER_ID, $COMPANY_ID, $sql ) );
		...
		while( $row = mysqli_fetch_array($res) )
		{
			...
			WLOG( sprintf( '[SET_RING %s:%s] Find DID %s Find Q %s RINGO %s COUNT %s', $MASTER_ID, $COMPANY_ID, $row[2], $row[0], $row[1], $EXT_CNT ) );
		}
		...
		WLOG( sprintf( '[SET_RING %s:%s] %s', $MASTER_ID, $COMPANY_ID, 'fwconsole reload' ) );


		mysqli_close ( $conn );
		mysqli_close ( $conn_db );

		WLOG( sprintf( '[SET_RING %s:%s] %s', $MASTER_ID, $COMPANY_ID, 'END   =======================================================================================================' ) );
	...
	if (isset($args->JSON_REQUEST)) {
		...
		error_log(sprintf('call setRingo.php args->JSON_REQUEST [%s]', $args->JSON_REQUEST));
		WLOG( sprintf( '[SET_RING CALL] setRingo.php args->JSON_REQUEST [%s]', $args->JSON_REQUEST ) );
		...
		if (is_object($JSON_REQUEST)) {
			...
			if (isset($JSON_REQUEST->REQ)) {
				if ($JSON_REQUEST->REQ == 'SET_RINGO') 
				{
					...
					$JSON_API_RESULT->JSON_RESULT->MESSAGE    = setRingo( $JSON_REQUEST->MASTER_ID, 
