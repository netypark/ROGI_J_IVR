...
include 'plog.php';
	...
	{ 
		...
		if( $row = mysqli_fetch_array($res) )
		{
			...
			**/
			...
			{
				$sql = "update trunks set disabled='off' where trunkid=$TR_ID;update pjsip set data='sip:${TR_NUM}@${TR_DOM}' where id=$TR_ID and keyword='client_uri';update pjsip set data='$TR_PAS' where id=$TR_ID and keyword='secret';update pjsip set data='$TR_DOM' where id=$TR_ID and keyword='from_domain';update pjsip set data='$NS_SVR_IP' where id=$TR_ID and keyword='sip_server';update pjsip set data='$TR_SVR_PORT' where id=$TR_ID and keyword='sip_server_port';update pjsip set data='off' where id=$TR_ID and keyword='disabletrunk';";
			}
			else
			{
				$sql = "update trunks set disabled='on' where trunkid=$TR_ID;update pjsip set data='sip:${TR_NUM}@${TR_DOM}' where id=$TR_ID and keyword='client_uri';update pjsip set data='$TR_PAS' where id=$TR_ID and keyword='secret';update pjsip set data='$TR_DOM' where id=$TR_ID and keyword='from_domain';update pjsip set data='$NS_SVR_IP' where id=$TR_ID and keyword='sip_server';update pjsip set data='$TR_SVR_PORT' where id=$TR_ID and keyword='sip_server_port';update pjsip set data='on' where id=$TR_ID and keyword='disabletrunk';";
			}
			error_log($sql);
			WLOG( sprintf( '[INS_KEYNUM %s] %s', $TR_NUM, $sql ) );
    ...
    \"CALL_ARGS\": { 
		...
		$sql = "select r_src from ROUTE2 where r_src='$TR_NUM';";
		...
		{
			error_log( sprintf('r_src [%s] ..!', $row[0]) );
			WLOG( sprintf( '[INS_KEYNUM %s] ROUTE2.r_src %s ..!', $TR_NUM, $row[0] ) );

			$SRC_NUM = $row[0];
			$is_already = 1;
			$sql = "update ROUTE2 set r_proxy_domain='$TR_DOM', r_proxy_addr='$TR_SVR_IP', r_udpport='$TR_SVR_PORT' where r_src='$TR_NUM';update modified_table set ROUTE2=1;";
		}
		else
		{
			error_log( sprintf('ROUTE2.r_src is NULL ..!' ) );
			WLOG( sprintf( '[INS_KEYNUM %s] ROUTE2.r_src is NULL ..! -> insert ', $TR_NUM ) );

			$sql = "insert into ROUTE2 ( r_type, r_src, r_method, r_proxy_domain, r_proxy_addr, r_udpport ) values ( 0, '$TR_NUM', 'REGISTER', '$TR_DOM', '$TR_SVR_IP', $TR_SVR_PORT );update modified_table set ROUTE2=1;";
